...
// Hook for fetching additional node data when we already have the basic node data
export const useNodeAdditionalData = (
  node: Node | null,
  organizationUuid: string,
  nodeUuid: string,
  scanSessionsLimit: number = 25,
  ...
  const [state, setState] = useState<Omit<UseNodeDataState, 'node'>>({
    cveData: null,
    eventsData: null,
    // tasksData: null, // Commented out for now
    scanSessionsData: null,
  ...
  useEffect(() => {
    const fetchAdditionalData = async () => {
      if (!node || !organizationUuid || !nodeUuid) {
        setState(prev => ({ ...prev, loading: false }));
        return;
      }
      ...
      try {
        ...
        setState(prev => ({
          ...prev,
          loading: false,
          error: error instanceof Error ? error.message : 'Failed to fetch additional node data',
        }));
      }
    };

    fetchAdditionalData();
  }, [node, organizationUuid, nodeUuid, scanSessionsLimit, scanSessionsOffset]);
